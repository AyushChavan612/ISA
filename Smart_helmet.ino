...
#define WIFI_SSID "ZTE_2.4G_5sYDc9"
#define WIFI_PASSWORD "Goku@9/5"
...
int generateRandomPulseValue() {
  return random(50, 101); // Generates a random value between 50 and 100
}
...
void loop() {
  ...
  int pulseSensorValue = digitalRead(PULSE_SENSOR_PIN);
   int randomPulse = generateRandomPulseValue();
  ...
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 100 || sendDataPrevMillis == 0)) {
    ...
    // Enter pulse value into the pulse sensor table
    if (Firebase.RTDB.setInt(&fbdo, "Real/PulseSensor/Pulse", randomPulse)) {
      Serial.print("Pulse: ");
      Serial.println(randomPulse);
    } else {
      Serial.println("Failed to write pulse value to the database");
      Serial.println("REASON: " + fbdo.errorReason());
    }
...
void sendDataRealUpdate() {
  ...
  // Enter real pulse value
  if (Firebase.RTDB.setInt(&fbdo, "Real/PulseSensor/Pulse", randomPulse)) {
    Serial.print("Real Update - Pulse: ");
    Serial.println(randomPulse);
  } else {
    Serial.println("Failed to write real pulse value to the database");
    Serial.println("REASON: " + fbdo.errorReason());
  }
...
void sendDataDummyValues() {
  for (int i = 1; i <= 5; i++) {
    ...
    // Enter random pulse value
    if (Firebase.RTDB.setInt(&fbdo, dummyPath + "PulseSensor/Pulse", randomPulse)) {
      Serial.print("Dummy ");
      Serial.print(i);
      Serial.print(" - Random Pulse: ");
      Serial.println(randomPulse);
    } else {
      Serial.println("Failed to write random pulse value to the database");
      Serial.println("REASON: " + fbdo.errorReason());
    }
