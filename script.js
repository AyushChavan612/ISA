...
var realPulseref = database.ref('Real/PulseSensor/Pulse');
...
realPulseref.on('value', function(snapshot) {
    var pulseValue = snapshot.val();
    document.getElementById('realPulse').innerHTML = pulseValue + " ";
}, function(error) {
    console.error('Error fetching force sensor data:', error);
});
...
for (let i = 1; i <= 5; i++) {
    (function(index) {
        ...
        const dummyPulseRef = database.ref('Dummy' + index + '/PulseSensor/Pulse');
        ...
        dummyPulseRef.on('value', function(snapshot) {
            const dummyPulse = snapshot.val();
            document.getElementById('dummyPulse' + index).innerHTML = dummyPulse + "";
        }, function(error) {
            console.error('Error fetching dummy force data:', error);
        });
    })(i);
